...
accou=10003
balance=requests.get('http://0.0.0.0:4669/api/fetch_customer_info/'+str(accou)).json()['response']['account_balance']
address=requests.get('http://0.0.0.0:4669/api/fetch_customer_info/'+str(accou)).json()['response']['address']
email=requests.get('http://0.0.0.0:4669/api/fetch_customer_info/'+str(accou)).json()['response']['email']
phone_number_user=requests.get('http://0.0.0.0:4669/api/fetch_customer_info/'+str(accou)).json()['response']['phone_number']
user_name=requests.get('http://0.0.0.0:4669/api/fetch_customer_info/'+str(accou)).json()['response']['first_name']
l_transaction=requests.get('http://0.0.0.0:4669/last_Transaction/'+str(accou)).json()['data']
k=len(l_transaction)
...
while True:
    ...
    response = conversation.message(
            ...
            message_input = {'text':'can you do netbanking'},
            context = context
            )
